...
//     // fn extract(&self, coords: Vec<i32>) ->PyResult<BlockSpace> {
//     // }

//     // fn remove(&mut self, coords: Vec<i32>) -> PyResult<()> {
//     //     let starts: Vec<i32> = self.blocks.iter().map(|[_, x, _]| *x).collect();
...
//     fn remove_abs(&mut self, coords: Vec<i32>) -> PyResult<()> {
//         let mut coords = coords;
...
//     fn insert_abs(&mut self, pos: i32, state: i32, length: i32) -> PyResult<()> {
...
//     fn append_abs(&mut self, state: i32, length: i32) -> PyResult<()> {
